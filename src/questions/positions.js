CoCreate.actions.init({
    ...
    callback: async (action) => {
        ...
        for (let i = 0; i < questions.length; i++) {
            currentPosition += 1
            if (questions[i].position === currentPosition && questions[i].position !== position)
                continue
            if (questions[i].name === name)
                continue
            if (questions[i].position === position && currentPosition === position)
                currentPosition += 1
        ...
        document.dispatchEvent(new CustomEvent('updatePositions', {
            detail: {}
        }));
    }
});
